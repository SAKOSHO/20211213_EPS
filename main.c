 ...
 * Author: 佐々木翔
 *
 * Created on 2021/11/07, 11:23
 * Discription　実装試験3の電源系のプログラム
 * 編集履歴
 * 2021/11/07：初版
 * 
 */
...
volatile char data1[8] = {5, 0, 0, 0, 0, 0, 0, 1};                               //HKデータ
volatile char data2[8] = {5, 0, 0, 0, 0, 0, 0, 2};                               //HKデータ
volatile char data3[8] = {5, 0, 0, 0, 0, 0, 0, 3};                               //HKデータ
volatile char data4[8] = {5, 0, 0, 0, 0, 0, 0, 4};                               //HKデータ
volatile char data5[8] = {5, 0, 0, 0, 0, 0, 0, 5};                               //HKデータ
volatile char data6[8] = {5, 0, 0, 0, 0, 0, 0, 6};                               //HKデータ
volatile char data7[8] = {5, 0, 0, 0, 0, 0, 0, 7};                               //HKデータ
...
{ 
    ...
    while(1)
    {
        ...
        {   
            ...
            Bit_Modify(_CANINTF, 0b00000001, 0b00000000);
        ...
        if(mode_cnt < 2000)
        {
            ...
            if(mode == _ChargeMode)
            {
                ...
                if(cnt1 >= rate2)
                {
                    ...
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA1, SIDL_W, EID8_DATA1, EID0_DATA1);//バッテリー電圧値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data1);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求


                    Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA2, SIDL_W, EID8_DATA2, EID0_DATA2);//バッテリー電流値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data2);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求

                    Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA3, SIDL_W, EID8_DATA3, EID0_DATA3); //framの電流値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data3);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求

                    Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA4, SIDL_W, EID8_DATA4, EID0_DATA4);//姿勢決定センサの電流値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data4);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求

                    Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA5, SIDL_W, EID8_DATA5, EID0_DATA5);//姿勢制御アクチュエータの電流値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data5);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求

                    Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA6, SIDL_W, EID8_DATA6, EID0_DATA6);//通信機器の電流値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data6);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求

                    Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                    Load_TX_ID(_F_TXB0SIDH, SIDH_DATA7, SIDL_W, EID8_DATA7, EID0_DATA7);//通信機器の電流値書き込み
                    Load_TX_Data(_F_TXB0D0, 8, data7);                                   //データ
                    RTS0(_CAN_BAUDRATE);                                                //送信要求
            ...
            // 充電モード移行命令
            if(mode_cnt == 1500)                                   //safe == 1はメッセージの送信回数を制御するため
            {
                mode = _ChargeMode;

                Write(_TXB0DLC, 0b00000001);                                            //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_CMD, SIDL_W, 0, 0);           //モード書き込みIDの設定
                Load_TX_Data(_F_TXB0D0, 1, &mode);                                      //モード書き込み
                RTS0(_CAN_BAUDRATE);                                                    //送信要
...
void CONFIG()
{
    ...
    Write(_RXM0SIDL, 0b11111110);                                               //マスク設定下位全一致
